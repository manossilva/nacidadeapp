...
/**
 * Carrossel de bolinhas com os locais num raio de 1.5km.
 * Props:
 *   items  — array de { ...place|event, _type, _distance }
 *   loading — boolean
 */
export default function NearbyCircles({ items, loading, onRequestLocation, hasLocation }) {
        ...
        >
          <div
            ...
            style={{
              ...
              border: '2px dashed rgba(114,140,104,0.5)',
            }}
          >
        ...
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="flex-shrink-0 flex flex-col items-center gap-2">
            <div
              ...
              style={{ background: 'rgba(212,191,160,0.4)' }}
            />
  ...
  if (loading) {
    return (
      <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} className="flex-shrink-0 flex flex-col items-center gap-2">
            <div className="w-16 h-16 rounded-full animate-pulse" style={{ background: 'rgba(212,191,160,0.5)' }} />
            <div className="h-2.5 w-12 rounded-full animate-pulse" style={{ background: 'rgba(212,191,160,0.4)' }} />
          </div>
        ))}
      </div>
  ...
  if (!items || items.length === 0) {
    return (
      <p className="text-sm py-2" style={{ color: '#9aad92' }}>
        Nenhum lugar encontrado num raio de 1,5 km.
      </p>
    )
  }
            ...
            >
              {item.cover_image_url ? (
                ...
                <div
                  ...
                  style={{ background: '#8f6e49', color: '#f5f0e8' }}
                >
                  {initials}
                </div>
              )}
              {/* Pulso para "em andamento" */}
              {item._type === 'event' && new Date(item.starts_at) <= new Date() && (
                <span
                  className="absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-white"
                  style={{ background: '#e35220' }}
                />
              )}
            </div>
            <div className="text-center w-16">
              <p className="text-[11px] font-medium leading-tight line-clamp-2" style={{ color: '#1f2e1a' }}>
                {item.name}
              </p>
              <p className="text-[10px] mt-0.5" style={{ color: '#9aad92' }}>
